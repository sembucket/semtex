...
static double splib_alp, splib_bet;
...
void jacgl (int n, double alpha, double beta, double *xjac)
{
  ...
  for (j = 1; j < nh; j++)
      ...
      for (k = 0; k < STOP && fabs(dx) > EPS; k++)
	{
      ...
      if (k == STOP) 
	fprintf (stderr, "splib: jacgl did NOT converge, resid = %g\n",
		 fabs(dx));
...
void jacgr (int n, double alpha, double beta, double *xjac)
{
  ...
  for(j = 0;j < np;j++)
      ...
      for (k = 1; k < STOP && fabs(dx) > EPS; k++)
	{
      ...
      if (k == STOP) 
	fprintf(stderr, "splib: jacgr did NOT converge, resid = %g\n",
		fabs(dx));
...
void jacg (int n, double alpha, double beta, double *xjac)
{
  ...
  for (j = 0;j < nh;j++)
      ...
      for (k = 1; k < STOP && fabs(dx) > EPS; k++)
	{
      ...
      if (k == STOP)
	fprintf (stderr, "splib: jacg did NOT converge, resid = %g\n",
		 fabs(dx));
