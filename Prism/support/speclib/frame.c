...
int Frame_init (Field *U, BSystem *B, int id, const char *name)
{
  FILE *fp;
  MatrixSet M;
  ...
  if((M->storage.fp = fp = fopen (fname, "rb")) == (FILE*) NULL ) {
    ...
    if (fp) {                    
      ...
      speclib_error("frame: unable to create a new matrix file");
  }    
  ...
  if (mset_load(M, B) == Unknown) {
    ...
    if (unlink(fname) == -1)
      speclib_error("frame: unable to remove a bad matrix file");
    else
...
void Frame_load (Field *U, BSystem *B, int id)
{
  ...
  if (!M)
    speclib_error
      ("Frame_load: no matrix for %c frame %d\n", FIELD_TYPE(U), id);
  ...
  case Unknown:
  default:
    speclib_error("Frame_load: don't know how to load the matrix for "
		  "frame %c.%d\n", FIELD_TYPE(U), id);
    break;
  }
  return;
...
{
  const int keep = option("keep");
  const int core = option("core");
  const int prep = option("prep");
  MatrixSet  M   = mset_get (id, U);
  ...
  default: 
    speclib_error("frame: ??? logic problem in Frame_save ???");
    break;
  }
  return;
}
...
static int mset_load (MatrixSet M, BSystem *B)
{
  ...
  int      nobj, nrows, nsolve, size_Hp, eipts;
  ...
  B->SC->A_22   = dvector (0, (eipts * (eipts+1)/2) * B->families - 1);
  ...
  nobj = (eipts * (eipts+1)/2) * B->families;
  ...
  for (k = 1; k < (*B).elements; k++) {
    ...
    B->SC[k].A_22   = B->SC->A_22     + (eipts * (eipts+1)/2) * M->famids[k]; 
  }
...
static int mset_save (MatrixSet M, BSystem *B)
{
  ...
  const int eipts   = B->ipts / nel;
  const int nsolve  = MAX(B->bdof, 1);
  const int size_Hp = (bw > 0) ? nsolve * (bw + 1) : nsolve * (nsolve + 1) / 2;

  size_t unit_H  = sizeof(double);
  FILE   *fp     = M->storage.fp;
  register int k, nrows, nobj;
 ...
 saveError:
  if (!(option ("core") && option ("keep")))
    speclib_error("frame: unable to save a matrix file");
  return M->status = Unknown;
}
...
void mset_show (void* Mptr)
{
  MatrixSet M = (MatrixSet) Mptr;
  fprintf(stderr, "Matrix %c.%d: status = ", M->type, M->id);
  switch (M->status) {
  case OnFile:
    fputs("on file", stderr);
    break;
  case InMemory:
    fputs("in memory", stderr);
    break;
  default:
...
static MatrixSet mset_add (int id, Field *U)
{
  ...
  sprintf (key, "%c%d", U->type, id);
  M = (MatrixSet) calloc(1, sizeof(struct ms_tag));

  M->famids   = ivector (0, nel-1);     /* initialize the MatrixSet */
  M->status   = Unknown;
  M->type     = U->type;
  M->id       = id;

  for (k = 0; k < nel; k++)                    /* get the family ID's */
    M->famids [k] = Family_get(U + k)->id;
...
void Frame_set (int frame_number, int nfields, ...)
{
  ...
  if (nfields < 1)
    speclib_error("Frame_set called with less than 1 frame");
...
{
  const int nel   = Field_count(U);
  const int index = U->ns * nel * frame;
  register int k;
