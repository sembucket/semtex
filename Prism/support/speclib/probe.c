 ...
 * $Id$
 *
 * A probe is used to sample a field at an arbitrary (x,y)-point inside
 * the computational domain.
 *
 * Copyright (c) 1994-1998 Ronald Dean Henderson
 * -------------------------------------------------------------------- */
...
static int GetRS (Element *elmt, double x, double y, double *r, double *s)
{
  double rr = *r;
  double ss = *s;
...
double Probe_eval (const Probe *p, const Field *u)
{
  ...
  case PROBE_XP:
	...
	ddot(nr, p->location.mesh.hr, 1, u[id].field[i], 1);
    break;
    
  case PROBE_INDEX: 
    val = u[id].field[p->location.node.is][p->location.node.ir];
    break;
  }
