...
static void getargs  (int, char*[],int*,int*,int*,int*,float*,FILE**,FILE**);
static void refile   (FILE*, FILE*, int*, float*);
static void pad      (float*, FILE*, int, float, int, int);
static void nondimen (float*, int);
static void normalize(float*, int, int);
static void printhead(FILE*, int, int, int, int, int, int, float, int, float);
static void printup  (FILE*, float*, int, float);
 ...
 *===========================================================================*/
{
  ...
  int      NPad, TranLen, TabLen;
  float    SampRate = 1.0, Mean = 0.0;
  register i;
   ...
   *-------------------------------------------------------------------------*/
  rewind(fp_Store);
  pad((float*)Zbuf, fp_Store, DeMean, Mean, NData, NPad);

  /*-------------------------------------------------------------------------*
   * Make autocorrelation as the backtransform of product of DFT & conjugate.*
   *-------------------------------------------------------------------------*/
  
  rcFFT(Zbuf, TranLen, Wtab, TabLen, FORWARD);
  ...
  rcFFT(Zbuf, TranLen, Wtab, TabLen, INVERSE);
  
  /*-------------------------------------------------------------------------*
   * Scale, print, exit                                                      *
   *-------------------------------------------------------------------------*/
  if (NonDim)
    nondimen((float*)Zbuf, TranLen << 1);
  else
    normalize((float*)Zbuf, TranLen, NData);
  ...
  printup(fp_out, (float*)Zbuf, NLag, SampRate);
...
static void pad(float *Data, FILE *fp,
		int DeMean, float Mean, int NData, int NPad)
/*===========================================================================*
...
static void nondimen(float *Data, int N)
/*===========================================================================*
...
static void normalize(float *Data, int N, int NData)
/*===========================================================================*
...
static void printhead(/* on    */ FILE  *fp,
		      /* using */ int    Verbose,
		      /* put   */ int    NData,
		                  int    NPad,
                                  int    NLag,
...
static void printup(FILE *fp, float *Data, int NLag, float SampRate)
/*===========================================================================*
