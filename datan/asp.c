...
static double setmask    (double *, char *, int);
static int    moreinput  (FILE *);
static void   datawindow (Complex *, double *, char *, int);
static void   sum        (Complex *, double *, int);
static double norm       (double *, double, double, int, int);
static void   printup    (FILE *, double *, char *, int, int, int, 
 ...
 * ========================================================================= */
{
  ...
  double   *windowmask, *autobuf;
  register  i, k;
  int       npts, navg, blocksize, blocksize_4, blocksize_2;
  double    mean, meanhat, samprate, Wss, variance;
  ...
  windowmask = dvector(0, blocksize - 1);
  autobuf    = dvector(0, blocksize_2);
  ...
  if (overlap) {
    /* --------------------------------------------------------------------- *
     * Do a startup half-read for overlap averaging.                         *
     * --------------------------------------------------------------------- */
...
static double setmask(double *mask, char *window, int block)
/* ========================================================================= *
 * Set up a mask for data-windowing.                                         *
 * ========================================================================= */
{
  int    j, block_2, tenpercent;
  double twopi_block, Wss;
...
static void sum (Complex *work, double *autobuf, int nyquist)
/* ========================================================================= *
...
static double norm(double *autobuf,
		   double  Wss, 
		   double  rate,
		   int     nyquist,
		   int     navg)
 ...
 * ========================================================================= */
{
  double    variance = 0.0, scale, deltaF;
  register  i;
 ...
 * ========================================================================= */
{
  ...
  double  deltaF = 0.5 * samprate / bs_2, F = 0.0;
  char    buf[BUFSIZ];
