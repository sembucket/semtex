/*****************************************************************************
 * memspec.c: all-pole spectral analysis.
 *
 * Synopsis:
 * ---------
...
static void  memcof    (real data[], int n, int m, real *xms, real d[]);
static real  evlmem    (real fdt, real d[], int m, real xms);
static void  getargs   (int argc, char *argv[], FILE **fp_in, 
	                real *Fstart, real *Fend, real *Fsamp,
			real *Diam,   real *Vel,  int *Neval, int *Npoles);
static void  getdata   (FILE *fp, real *Data, int *Ndata, int MAX);
static void  printhead (FILE *fp, real Fs, real Fe, int Nd, int Np);
...
{ 
  ...
  int   NPoles = DEFPOL;
  real *Poles;
  real  NormFactor, Gain, FreqInt, Freq, FDelta, SDen;

  int   NData;
  real *Data = rvector (1, MAXPNT);
...
static void printhead (FILE *fp, real Fstart, real Fend, int Nd, int Np)
/* ------------------------------------------------------------------------- *
