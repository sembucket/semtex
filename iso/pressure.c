...
/*===========================================================================*/
{
  int k1, k2, k3, b1, b2, K, N, FOURKon3;
  ...
  truncation(P, Dim);		/* Set all high-wavenumber stuff zero. */
  ...
  convolve(V[1], V[1], VV, WK, Wtab, Stab, Dim);
  for (k1=1; k1<K; k1++) {
    b1 = N-k1;
  ...
  convolve(V[1], V[2], VV, WK, Wtab, Stab, Dim);
  for (k1=1; k1<K; k1++) {
    b1 = N-k1;
    for (k2=1; k2<K && k1+k2<=FOURKon3; k2++) {
      b2 = N-k2;
  ...
  convolve(V[1], V[3], VV, WK, Wtab, Stab, Dim);
  for (k1=1; k1<K; k1++) {
    b1 = N-k1;
    for (k2=1; k2<K && k1+k2<=FOURKon3; k2++) {
      b2 = N-k2;
  ...
  convolve(V[2], V[2], VV, WK, Wtab, Stab, Dim);
  for (k1=1; k1<K; k1++) {
    b1 = N-k1;
  ...
  convolve(V[2], V[3], VV, WK, Wtab, Stab, Dim);
  for (k1=1; k1<K; k1++) {
    b1 = N-k1;
    for (k2=1; k2<K && k1+k2<=FOURKon3; k2++) {
  ...
  convolve(V[3], V[3], VV, WK, Wtab, Stab, Dim);
  for (k1=1; k1<K; k1++) {
