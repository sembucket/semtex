///////////////////////////////////////////////////////////////////////////////
// NS.C: integrate unsteady momentum equations forward in time.
//
// Copyright (c) Hugh Blackburn 1999
//
// This version incorporates eddy-viscosity SGS stress, with the
// dynamic algorithm.  This is handled within the framework of the
...
// ---------------------------------------------------------------------------
{
  ...
  for (i = 0; i < DIM; i++) {
    ...
    for (j = 0; j < nOrder; j++) {
      *(Us[i][j] = new AuxField (new real [nTot], nZ, D -> elmt)) = 0.0;
      *(Uf[i][j] = new AuxField (new real [nTot], nZ, D -> elmt)) = 0.0;
    }
  }

  // -- Create extra storage needed for computation of SGSS, nonlinear terms.

  matrix<real> Ut (14, Geometry::nTotProc());
  ...
  while (D -> step < nStep) {
    ...
    // -- Compute SGSS, with modified eddy viscosity.

    SGSS (D, Us, Uf, Ut);

    // -- Unconstrained forcing substep.

    nonLinear (D, Us, Uf, Ut, ff);
