  ...
  elmt (E)
{
  ...
  for (i = 0; i < nfield; i++) {
    udat[i] = new real_t [ntot];
    u[i]    = new Field (b[i], udat[i], nz, E, field[i]);
    ua[i]   = u[i];
  }
  
  // ** NEW ** Add a new AuxField to hold kinematic viscosity.

  ua[nfield] = new AuxField (new real_t [ntot], nz, E, 'l');
...
{
  int_t       i;
  const int_t nF = this -> ua.size();
  char        restartfile[StrMax];
  
  for (i = 0; i < nF; i++) *ua[i] = 0.0;
  ...
  if (file) {
    ...
    file.close();
    transform (FORWARD);
    ROOTONLY for (i = 0; i < nF; i++) ua[i] -> zeroNyquist();
  } else
...
// processor.
//
// ** NEW** Note we can just write put ua directly now.
// ---------------------------------------------------------------------------
{
  writeField (strm, D.name, D.step, D.time, D.ua);
...
//
// ** NEW ** We don't need to read in viscosity as it gets computed.
// ---------------------------------------------------------------------------
{
