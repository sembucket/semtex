...
Vector Field::xytangTractionNN (Field*     U ,
				AuxField** dU,
				AuxField** dV)
// ---------------------------------------------------------------------------
// Compute tangential tractive forces at each point on wall boundaries
// and sum to give vector force on boundary. dU, dV are derivatives of U and V
// multiplied by viscosity and must be in Fourier space
// ---------------------------------------------------------------------------
{
  const vector<Boundary*>& UBC    = U -> _bsys -> BCs (0);
  const int_t              np     = Geometry::nP();
  ...
  for (i = 0; i < nbound; i++) {
    secF = UBC[i] -> tangTractionNN ("wall", du);
    F.x -= secF.x;
    F.y -= secF.y;
  }
  
  // next lines are for total force, not force/length
  
  //  const real_t zFac = Femlib::value ("TWOPI / BETA ");
  //  F.x *= zFac;
  //  F.y *= zFac;
...
Vector Field::ztangTractionNN (Field*     W,
			       AuxField** dW)
// ---------------------------------------------------------------------------
// Compute tangential tractive forces at each point on wall boundaries
// and sum to give vector force on boundary. dW contains derivatives of W
// multiplied by viscosity and must be in Fourier space
// ---------------------------------------------------------------------------
{
  const vector<Boundary*>& WBC    = W -> _bsys -> BCs (0);
  const int_t              np     = Geometry::nP();
  const int_t              nz     = Geometry::nZProc();
  const int_t              nbound = W -> _nbound;
