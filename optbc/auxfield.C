...
//Note that in boundary optimization, 1/2A factor is removed.
// ---------------------------------------------------------------------------
{
...
real_t AuxField::mode_L2_mixed (const real_t* u1, const real_t* u2) const
// ---------------------------------------------------------------------------
// Return mixed_norm of u and anotheru
// ---------------------------------------------------------------------------
{
	const char  routine[] = "AuxField::mode_L2_mixed";
	const int_t nel  = Geometry::nElmt();
	const int_t npnp = Geometry::nTotElmt();
	real_t      Ek = 0.0;
	int_t       i;
	...
	for (i = 0; i < nel; i++) {
		E      = _elmt[i];
		Ek    +=  (E -> norm_L2_mixed (u1 + i*npnp, u2 + i*npnp));
		if (_nz > 1) 
			Ek  +=  (E -> norm_L2_mixed (u1 + i*npnp + NP, u2 + i*npnp + NP));
	}
...
real_t AuxField::mode_L2_mixed_weight (const real_t* u1, const real_t* u2,  const real_t* weight) const
// ---------------------------------------------------------------------------
// Return mixed_norm of u and anotheru
// ---------------------------------------------------------------------------
{
	const char  routine[] = "AuxField::mode_L2_mixed_weight";
	const int_t nel  = Geometry::nElmt();
	const int_t npnp = Geometry::nTotElmt();
	real_t      Ek = 0.0;
	int_t       i;
	...
	for (i = 0; i < nel; i++) {
		E      = _elmt[i];
		Ek    +=  (E -> norm_L2_mixed_weight (u1 + i*npnp, u2 + i*npnp,  weight + i*npnp));
		if (_nz > 1) 
			Ek  +=  (E -> norm_L2_mixed_weight (u1 + i*npnp + NP, u2 + i*npnp + NP, weight + i*npnp));
	}
...
AuxField& AuxField::updatelagrangeinterpolation (const int_t   nSlice ,
			    const real_t* basedata,
			    const real_t  time   ,
			    const real_t  period )
// ---------------------------------------------------------------------------
//linear interpolation to reconstruct the baseflow
// ---------------------------------------------------------------------------
{
  if (nSlice < 2) return *this;
...
AuxField& AuxField::updatelagrangeinterpolation_nonperiodic (const int_t   nSlice ,
			    const real_t* basedata,
			    const real_t  time   ,
			    const real_t  period )
// ---------------------------------------------------------------------------
//linear interpolation to reconstruct the baseflow
// ---------------------------------------------------------------------------
{  const char  routine[] = "AuxField::updatelagrangeinterpolation_nonperiodic()";
  if (nSlice < 2) return *this;

  const int_t  nPlane = Geometry::planeSize();
      const real_t interval = period / (nSlice-1);
    const real_t p        = fmod (time, interval) / interval;
    real_t       wm1, wp0, wp1, wp2;
    int_t        im1, ip0, ip1, ip2;
    int_t i;
...
if (i>nSlice-1) message (routine, "nonperiodic base flow is not long enough", ERROR);

if (i==0){
Veclib::smul (nPlane, 1-p, basedata + 0 * nPlane , 1, _data, 1);
Blas::axpy   (nPlane, p  , basedata + 1 * nPlane , 1, _data, 1);
	
	}
else if (i==nSlice-2) {
    Veclib::smul (nPlane, 1-p, basedata + (nSlice-2) * nPlane , 1, _data, 1);
    Blas::axpy   (nPlane, p  , basedata + (nSlice-1) * nPlane , 1, _data, 1);
}
 else {
    Veclib::smul (nPlane, wm1, basedata + im1 * nPlane , 1, _data, 1);
    Blas::axpy   (nPlane, wp0, basedata + ip0 * nPlane , 1, _data, 1);
...
void AuxField::meshXY (real_t* X, 
                       real_t* Y,
					   int_t& nX, 
					   int_t& nY)
// to get mesh points and store in X[nX] and Y[nY] 
{const int_t nel    = Geometry::nElmt();
const int_t npnp    = Geometry::nTotElmt();
const int_t np    = Geometry::nP();
 int i,j,k,mx=0,my=0;
 ...
 Veclib::zero (np*nel,     X, 1);
  Veclib::zero (np*nel,     Y, 1);
 for (i=0;i<nel;i++){
 xnpnp=_elmt[i]->xmesh();
 ynpnp=_elmt[i]->ymesh();
  ...
  {k=0;
   ...
   while (k<mx)
   {if(0.000001 > xnpnp[j]-X[k] && -0.000001 < xnpnp[j]-X[k] ) break;
	else  k++;  
   } 
   if(mx==0) {X[0]=xnpnp[j]; mx++;}
   if(k==mx) {X[k]=xnpnp[j]; mx++;}
  
  // to get y; 
  k=0;
   while (k<my)
   {if(0.000001 > ynpnp[j]-Y[k] && -0.000001 < ynpnp[j]-Y[k] ) break;
	else  k++;  
   } 
   if(my==0) {Y[0]=ynpnp[j]; my++;}
   if(k==my) {Y[k]=ynpnp[j]; my++;}
      }
  }
  nX=mx;
  nY=my;
...
real_t AuxField::EnergyK ( real_t modek, 
                           real_t* X, 
                           real_t* Y, 
                           int_t nX, 
                           int_t nY,
						   real_t* Ukr)
 // to calcuate the energy of u in axial mode modek. X[nX], Y[nY] are x, y coordiates of the mesh. 
{const int_t    nel = Geometry::nElmt();
 const int_t npnp    = Geometry::nTotElmt();
 const int_t    np   = Geometry::nP();
 Element* E;
 int_t i, j, ex,ey,nx,ny;
 real_t radiusa=0, radiusb=0; // intermedium to calculate the radius of the perturbation
 real_t dx,dy; // used in integration.
 real_t* u=_plane[0]; // the velocity in plane 0.
 real_t energyk=0; // the energy in mode k, final result.
 real_t* xnpnp;  //x of nodes in an element.
 real_t* ynpnp; // y of nods in an element.
 real_t uk[nY][nX]; // velocity at global nodes without overlapping.

 
 real_t* ukreal; // real part of the first integration
 real_t* ukimag; // imaginary part of the frst intergration.
 ukreal = new real_t [static_cast<size_t>(nY)];
 ukimag = new real_t [static_cast<size_t>(nY)];


 Veclib::zero(nY,ukreal,1);
 Veclib::zero(nY,ukimag,1);

 // loop over every element to create uk[nY][nX].
 for(i=0;i<nel;i++){
 xnpnp=_elmt[i]->xmesh();
 ynpnp=_elmt[i]->ymesh();
 ex=0; // position of the first node of this element in X.
 ey=0; // position of the first node of this element in Y.
 
 //locate the first node in the element to be X[ex],Y[ey].
 while(ex<nX){
  if (0.000001 > *xnpnp-X[ex] && -0.000001 < *xnpnp-X[ex])
  break;
  else ex++;
  }
 while(ey<nY){
  if (0.000001 > *ynpnp-Y[ey] && -0.000001 < *ynpnp-Y[ey])
  break;
  else ey++;
  }
 
 // loop over all the nodes in this element.
 for(j=0;j<npnp;j++,u++){
  nx=ex+j%np; // position of node j of this element in X.
  ny=ey+j/np; // position of node j of this element in Y.
  uk[ny][nx]=*u;
  } 
 }

// create ukreal[nY] and ukimag[nY]. ukreal(y)= integrate uk(x,y)cos(kx)/Lx from X[0] to X[nX-1].
// ukimag(y)=integrate uk(x,y)sin(kx)/Lx from X[0] to X[nX-1].
for(i=0;i<nY;i++)
for(j=0;j<nX;j++)
{if (j==0) dx=X[1]-X[0];
 else if (j==nX-1) dx=X[nX-1]-X[nX-2];
 else dx=(X[j+1]-X[j-1])/2;
 ukreal[i]+=uk[i][j]*cos(modek*X[j])*dx;
 ukimag[i]+=uk[i][j]*sin(modek*X[j])*dx;
}
ukreal[i]/=(X[nX-1]-X[0]);
ukimag[i]/=(X[nX-1]-X[0]);


// Ukr(i)=sqrt(ukreal^2+ukimag^2).
// calcuate energyk=integrate Ukr^2/Ly from Y[0] to Y[nY-1]. 

for(i=0;i<nY;i++)
{if (i==0) dy=Y[1]-Y[0];
 else if (i==nY-1) dy=Y[nY-1]-Y[nY-2];
 else dy=(Y[i+1]-Y[i-1])/2;
Ukr[i]=sqrt(ukreal[i]*ukreal[i]+ukimag[i]*ukimag[i]);
energyk+=Ukr[i]*Ukr[i]*dy;
}
energyk=energyk/(X[nX-1]-X[0])/(X[nX-1]-X[0])/(Y[nY-1]-Y[0])/2;
...
real_t y,x;
 ...
 for (j=0, src = _plane[k]; j<nel; j++){
	    ynpnp = _elmt[j] -> ymesh();
        xnpnp=  _elmt[j] -> xmesh();
		for (i=0; i<npnp; i++, src++){
		 y=*(ynpnp+i);
		 x=*(xnpnp+i);
		 //if (y>=1) *src=1.0/y/y*(*src);
		  if (y<=10) *src=0;
	   } 
 }
}
...
void AuxField::meshxymass (real_t* X, 
                          real_t* Y)
// to get mesh points and store in X, Y and Mass=dx.dy  
{   const int_t nel    = Geometry::nElmt();
	const int_t npnp    = Geometry::nTotElmt();
	int i;
	...
	for (i=0;i<nel;i++){	
		Veclib::copy (npnp, _elmt[i]->xmesh(),    1, X+i*npnp,    1);
		Veclib::copy (npnp, _elmt[i]->ymesh(),    1, Y+i*npnp,    1);
		}
