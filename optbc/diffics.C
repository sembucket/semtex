...
//load base and restart files  
 ...
 // Allocate memory for adjoint_integration, uc and lagragian gradient.
  ...
  for (i=0; i<NPERT; i++)        u2[i] = alloc_adjoint+ (NPERT+i)  * NZ * NP;
	...
	char filename1[StrMax], filename2[StrMax];	

	const int_t NC = Geometry::nPert();
	
	for(i=0;i<351;i++){
		//ifstream readfile;	
		sprintf   (filename1, "pert.fld.%d",i*200);
		sprintf   (filename2, "lns.fld.%d",i*200);
		...
		for (k=0;k<NPERT;k++)
			for(j=0;j<NZ;j++)
				Veclib::copy (NP, domain -> u[k] -> plane (j), 1, u1[k]+j*NP, 1);
		...
		similarity<<i*200<<setw(15)<<normv_mixed/sqrt(normv1*normv2)<<endl;
	}
	similarity<<endl;
