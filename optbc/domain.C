  ...
  elmt (E)
{
  ...
  alloc = new real_t [static_cast<size_t> (2*(nbase + 1) * nplane)];
  for (i = 0; i < nbase + 1; i++) {
...
// ---------------------------------------------------------------------------
{
  ...
  if (nSlice > 1) {		// -- Prepare for Fourier reconstruction.
    ...
    if (period < EPSDP)
  if (Femlib::ivalue ("FLinterpolation")==2)	Femlib::value ("BASE_PERIOD", period = dt ); 
  else Femlib::value ("BASE_PERIOD", period = dt * i / (i - 1.0));
...
void Domain::loadinstantu(vector<AuxField*>   instant_u, 
						              int_t   step)
//read in the instant velocity field
{   const char  routine[] = "Domain::loadinstantu";
    
    const int_t NPERT = Geometry::nPert();
    const int_t ntot    = Geometry::nTot();
	char        utoread[StrMax], msg[StrMax];
	Header   H;
	int_t    i, j;
	real_t*  addr;
	real_t   t0, dt = 0;
	...
	sprintf   (msg, ".fld.%d", step);
	ifstream file (strcat (strcpy (utoread, name), msg));
	
	if (!file) 		message (routine, "can't open instant velocity file", ERROR);

     file >> H;
	 for (j = 0; j < NPERT; j++) {
		addr = instant_u[j] -> plane(0);			
