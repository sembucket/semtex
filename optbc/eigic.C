...
// estimates of those in the original space in a siple way: the
...
static void weightfunction ( real_t*);
int main (int    argc,
	  char** argv)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
{
  ...
  const int_t NZ = Geometry::nZ();
	...
	int_t sizecontrolbc=1;
	vector<real_t*>  adjoint_integration;
	vector<real_t*>  uc;
	adjoint_integration.resize(ND);
	uc.resize(ND);
	
	real_t*     alloc_adjoint = new real_t [static_cast<size_t>(ND*NZ*sizecontrolbc*2)];
	for (i=0; i<ND; i++)        adjoint_integration[i] = alloc_adjoint+ i          * NZ * sizecontrolbc;
	for (i=0; i<ND; i++)                         uc[i] = alloc_adjoint+(i+1*ND) * NZ * sizecontrolbc;
  ...
  case PRIMAL:			// -- Forward in time.
    integrate (linAdvect , domain, analyst, adjoint_integration, uc); break;

  case ADJOINT:			// -- Backward in time.
    integrate (linAdvectT, domain, analyst, adjoint_integration, uc); break;
...
// ---------------------------------------------------------------------------
{
  ...
  if (icon == 0) {
    ...
    EV_big (task, Tseq, Kseq, ntot, kdim, 1, zvec, wr, wi, runinfo);
    for (j = 0; j < nvec; j++) {
      src = Kseq[j];
      for (i = 0; i < ND; i++)
	for (k = 0; k < NZ; k++)
...
static void weightfunction ( real_t* tgtw)
{int_t       i, k;
  const int_t ND = Geometry::nPert();
  const int_t NP = Geometry::planeSize();
  const int_t NZ = Geometry::nZ();
  for (i = 0; i < ND; i++)
    for (k = 0; k < NZ; k++){
	  domain ->u[i] -> weight (k);
	  domain -> u[i] -> getPlane (k, tgtw + (i*NZ + k)*NP);
    }	
