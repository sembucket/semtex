...
static void Zerodomain (Domain*);

void integrate (void            (*Advection)(Domain*, AuxField**, AuxField**, vector<AuxField*>, vector<AuxField*>, int_t),
		Domain*         D   ,
		StabAnalyser*   A   ,
		vector<real_t*>& adjoint_integration  , 
		vector<real_t*>& uc,
		vector<real_t*>& av_fields,
	    real_t* E_fields,
		real_t* massweight,
        int_t LEVEL )
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
{
  ...
  const bool   forwards = (Advection == linAdvect_unsteady) ? true : false;
  ...
  for (i = 0; i < NORD; i++)file://localhost/Users/xmao/opt/cyl/test/unsteady0.fld.58
    for (j = 0; j < NPERT; j++) {
      *Us[i][j] = 0.0;
      *Uf[i][j] = 0.0;
	}
  ...
  //initialized to zero:
   Zerodomain (D);
  //allocate memory for the instant velocity vector
   vector<AuxField*>   instant_u; 
   vector<AuxField*>   oscillation_u; 
   
  instant_u.resize  (NPERT);
  oscillation_u.resize  (NPERT);
   real_t*     alloc_instant_u = new real_t [static_cast<size_t>(NPERT*ntot*2)];
	for (j = 0; j < NPERT; j++) {
   	  instant_u[j]    =  new AuxField (alloc_instant_u + j * ntot, NZ, D -> elmt);
   	  oscillation_u[j]    =  new AuxField (alloc_instant_u + (j+NPERT) * ntot, NZ, D -> elmt);	
	}
	...
	//update the base flow from storage.	
	D -> updateBase();
		
	if (LEVEL ==2 ) {
			D -> loadinstantu(instant_u, D -> step); 
	        if ( Femlib::ivalue ("nonlinear_advection") ==1 ) {*D->U[0] += *instant_u[0]; *D->U[1] += *instant_u[1]; }
	}
	if (LEVEL==3) {
		if (D ->step ==nStep)       Veclib::fill (ntot*NPERT, 0 , instant_u[0]->plane(0), 1); 
		else                		D -> loadinstantu(instant_u, nStep- D -> step); 
		for (i = 0; i < NBASE ; i++) 
			Veclib::svvpt (NP, 2, instant_u[i] -> plane (0), 1, D->U[i] -> plane (0), 1, oscillation_u[i] -> plane (0), 1);
		
		if ( Femlib::ivalue ("nonlinear_advection") ==1 ) {*D->U[0] += *instant_u[0]; *D->U[1] += *instant_u[1]; }
		
		for (i = 0; i < NPERT ; i++){
			Veclib::svtvp (ntot, -2, av_fields[i], 1, oscillation_u[i] -> plane (0), 1, oscillation_u[i] -> plane (0), 1); 	
		    Veclib::vmul (NP, massweight, 1, oscillation_u[i] -> plane (0), 1, oscillation_u[i] -> plane (0), 1);
			//consider the contribution of toutflow to matrices.
			MS[i] -> Contribution_toutflow(D->U); //only activated for backward integration.
		}
    	if ( Femlib::ivalue ("nonlinear_advection") ==1) {*D->U[0] -= *instant_u[0]; *D->U[1] -= *instant_u[1]; }
	}
  ...
  if (Geometry::cylindrical() && 
    ...
    Advection (D, Us[0], Uf[0],instant_u, oscillation_u, LEVEL); // Uf[0]: (N(u)*r,N(v)*r,N(w)); Us[0] :(u,v,w); in forwards instant_u is not used to save u+U; in backwards, oscillation_u saves weightfactor*(2u-2u_av).
	if ( LEVEL==2 && Femlib::ivalue ("nonlinear_advection") ==1 ) {*D->U[0] -= *instant_u[0]; *D->U[1] -= *instant_u[1]; }
   ...
   // -- Complete unconstrained advective substep and compute pressure.
    if (Geometry::cylindrical()) { Us[0][0] -> mulY(); Us[0][1] -> mulY(); }//Us[0]:(ur,vr,w);
   // Compute v^.
    waveProp (D, const_cast<const AuxField***>(Us),
	         const_cast<const AuxField***>(Uf)); // D: (u^*r,v^*r,w^);

    for (i = 0; i < NPERT; i++) AuxField::swapData (D -> u[i], Us[0][i]);//Us[0]: (u^*r,v^*r,w^); D:(ur,vr,w); 

    rollm     (Uf, NORD, NPERT);
    setPForce (const_cast<const AuxField**>(Us[0]), Uf[0]);// Uf[0]=f*r (f=div*div P)
    Solve     (D, NPERT,  Uf[0][0], MS[NPERT], forwards);
    project   (D, Us[0], Uf[0]); //Uf[0]= (fx*r,fy*r,fz*r); f=grad(p)/nu - v^/nu/dt;
	...
	if ( Femlib::ivalue ("nonlinear_advection") ==1 && !forwards) {*D->U[0] += *instant_u[0]; *D->U[1] += *instant_u[1]; }
...
// A -> analyse (Us[0]);
	...
	//post-process of the integration of uc
	  if(LEVEL ==1 ) { 
		  // -- Process results of this step.
		
		  if (D->step ==1) 	{*(E_fields+1)=0; *(E_fields+3)=0;  Veclib::fill (ntot*NPERT, 0 , av_fields[0], 1);}

		  for (i = 0; i < NBASE ; i++) 
				*(E_fields+3) += D -> U[i] -> mode_L2_mixed_weight (D -> U[i] -> plane (0), D -> u[i] -> plane (0), massweight) / nStep;
		  for (i = 0; i < NPERT ; i++) {
				*(E_fields+1) += D -> u[i] -> mode_L2_mixed_weight (D -> u[i] -> plane (0), D -> u[i] -> plane (0), massweight) / nStep;
    			Veclib::svtvp (ntot, 1.0/nStep, D -> u[i] -> plane (0), 1, av_fields[i], 1, av_fields[i], 1); 	
		  }
			D->dumpperstep();		
	  }
	//integration of the gradient(direction)
	 if(LEVEL ==2 ) { 
			if (D->step ==1) 		 {*(E_fields+2)=0; *(E_fields+4)=0; *(E_fields+5)=0; Veclib::fill (ntot*NPERT, 0 , av_fields[0], 1);}
			
			for (i = 0; i < NBASE ; i++) 
				*(E_fields+4) += D -> U[i] -> mode_L2_mixed_weight (D -> U[i] -> plane (0),    D -> u[i] -> plane (0), massweight) / nStep;
		    for (i = 0; i < NPERT ; i++) {
				*(E_fields+2) += D -> u[i] -> mode_L2_mixed_weight (D -> u[i] -> plane (0),    D -> u[i] -> plane (0), massweight) / nStep;
				*(E_fields+5) += D -> u[i] -> mode_L2_mixed_weight (D -> u[i] -> plane (0), instant_u[i] -> plane (0), massweight) / nStep;	
				Veclib::svtvp (ntot, 1.0/nStep, D -> u[i] -> plane (0), 1, av_fields[i], 1, av_fields[i], 1); 	
			}
	}
}

	if (NPERT==3  && Femlib::ivalue ("BETA") !=0)  {*(E_fields+3)=0; *(E_fields+4)=0;}		
    delete[]     alloc_instant_u;
	for (j = 0; j < NPERT; j++) {
		 delete  instant_u[j]   ;
		 delete  oscillation_u[j]   ;
	}
	
}


void linAdvect_unsteady (Domain*    D ,
		AuxField** Us,
		AuxField** Uf,
		vector<AuxField*>   instant_u,
        vector<AuxField*>   oscillation_u,
...
//                 
//           N  = - ( U.grad u + u.grad U ) // N(0)=N(0)*r, N(1)=N(1)*r
//           ~        ~      ~   ~      ~
...
if ( Femlib::ivalue ("nonlinear_advection") ==1 && LEVEL==1){                          // for the nonlinear solvers
//add Criolis force	
	if (Geometry::cylindrical()  && NPERT == 3) {
			*N[2] += T -> times (*u[2], *u[1]) . divY();
			N[1] -> axpy (-1.0, T -> times (*u[2], *u[2]));
	}
...
void linAdvectT_unsteady (Domain*    D ,
		 AuxField** Us,
		 AuxField** Uf,
		 vector<AuxField*>   instant_u,
		 vector<AuxField*>   oscillation_u,
...
if ( Femlib::ivalue ("nonlinear_advection") ==1){  
	// -- Centrifugal, Coriolis terms for cylindrical coords.
	if (Geometry::cylindrical()  && NPERT == 3) {
		   *N[2] += T -> times (*u[2], *instant_u[1]) . divY();	
		   *N[1] += T -> times (*u[2], *instant_u[2]);
			N[2] -> axpy (-2.0, T -> times (*u[1], *instant_u[2]) . divY() );
	}	
	// -- N_i -= u_j d(u^*_i) / dx_j.
	for (i = 0; i < NPERT; i++)
			for (j = 0; j < NPERT; j++) {
				...
				N[i] -> timesMinus (*T, *instant_u[j]);
			}
		
		// -- N_i += u^*_j d(u_j) / dx_i; here u is u^* in the meth and instant_u is u in the meth.
		
		for (i = 0; i < NPERT; i++)
			for (j = 0; j < NPERT; j++) {
				(*T = *instant_u[j]) . gradient (i);
				if      (Geometry::cylindrical() && i <  2) T -> mulY();
    ...
    *N[i] *= -1.0;
	...
	//additional terms from forward integration, stemming from L
	for (i = 0; i < NPERT; i++){
		if (Geometry::cylindrical() && i < 2) oscillation_u[i] ->mulY();
		*N[i] += *oscillation_u[i];
	}
}
...
// ---------------------------------------------------------------------------
{
  ...
  for (i = 0; i < NPERT; i++) {
    ...
    if (Geometry::cylindrical() && i == 2) Uf[i] -> mulY();          // force_u and force_v have been multiplied by r.
    *Uf[i] *= alpha;
  }
...
static MatrixSys** preSolve (const Domain* D, bool forwards)
// ---------------------------------------------------------------------------
...
// it it too expensive to update the matrices at every step. therefore for nonlinear study adopt JACPCG for velocity all the time (this also holds true for unsteady baseflow).
if ( (Femlib::ivalue ("nonlinear_advection") ==1 || Geometry::nSlice()>1)      &&     !forwards)    method = JACPCG;
  ...
  if (found) {
    ...
    M = new MatrixSys (lambda2, betak2, bmode, D -> elmt, D -> b[1], method, D -> U, forwards);
    MS.insert (MS.end(), M);
    system[1] = M;
    cout << ((method == DIRECT) ? '*' : '&') << flush;
  }
  ...
  if (NPERT == 3) {
    ...
    if (found) {
      ...
      M = new MatrixSys (lambda2, betak2, bmode, D -> elmt, D -> b[2], method, D -> U, forwards);
      MS.insert (MS.end(), M);
      system[2] = M;
      cout <<  ((method == DIRECT) ? '*' : '&') << flush;
    }
  ...
  M      = new MatrixSys (0.0, betak2,bmode, D -> elmt, D -> b[NPERT], method, D -> U, forwards);
  MS.insert (MS.end(), M);
  system[NPERT] = M;
...
// ---------------------------------------------------------------------------
{
  ...
  if (i < NPERT && step < NORD) {
	...
	if (!forwards) tmp -> Contribution_toutflow(D->U); 
    
	D -> u[i] -> solve (F, tmp, D -> U, forwards);
    delete tmp;

  } else D -> u[i] -> solve (F, M, D -> U, forwards);
    
}



static void Zerodomain (Domain* D)
// ---------------------------------------------------------------------------
