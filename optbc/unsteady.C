...
average_base(av_base,E_base,Eav_base, massweight);	
	...
	for (j = 0; !converged && j < iterations; j++) {
		if (j>0){
			...
			if (abs(steplengthcommand) > EPSDP) steplength= steplengthcommand; // if steplength is given from command line, use it.		
			...
			while(unsteadiness[j]>unsteadiness[j-1] && ! (abs(steplengthcommand) > EPSDP)  && steplength > 1.0e-10);
