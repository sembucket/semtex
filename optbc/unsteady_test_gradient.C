...
average_base(av_base,E_base,Eav_base, massweight);	
	...
	for (j = 0; !converged && j < iterations; j++) {
		if (j>0){
			// get the direction from the gradient
				...
				*E_total  = *E_base   + 2* *E_base_pert   + *E_pert;
				...
				growthfile<< j<<setw(15)<<steplength<< setprecision(15) << setw(25)<<unsteadiness[j]<<setw(25)<<unsteadiness[j]-unsteadiness[j-1] <<setw(25)<<normlag[j-1]*steplength <<endl;
		...
		if (abs(steplength)<EPSDP  && j>0) {converged=1; printf("converged !\n"); growthfile<<"converged!"<<endl;} 
...
{   const char  routine[] = "choose_step_length";
	...
	printf("steplength =%e ,  unsteadiness[i-1]= %e  , normdirection = %e \n ", steplength, min, norm_direction);
}
