...
//
// This version, modified for use with scat, differs from standard semtex
// because it has to accumlate correlations for passive scalar.
//
...
//                      \ .  .  .  ww /     \  .  .  .  J /
//
// Note the structure is different/names conflict compared to the
// plain non-scalar case. Even for 2C there will be 6 correlations,
// and 10 for 3C. Fortunately the code modifications required are
// fairly minimal. Note also that we have dropped AVERAGE = 3, or
// correlations for the energy balance.
//
///////////////////////////////////////////////////////////////////////////////
...
// ---------------------------------------------------------------------------
{
  ...
  if (_iavg > 1) {
    ...
    if (_nvel == 4) (*wrka[3] = *_raw['w']) . transform (INVERSE);
