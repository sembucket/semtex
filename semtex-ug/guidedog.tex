...
Our boundary conditions should be what we expect for a perturbation
field at the boundaries of our solution domain.  Typically this means
applying boundary condition sets appropriate to no-slip walls at all
domain boundaries except at outflow boundaries in open flows, where a
stress-free type is normally used.  We will consider examples in
chapter~\ref{ch.stab}.
...
adjoint \NavSto\ equations may be written (semi-symbollically)
\begin{equation}
\partial_t\uadj =
-\left[\bm{I}-\bm{\nabla}\nabla^{-2}\bm{\nabla\cdot}\right]
\left[-\Ubase\bm{\cdot\nabla}
...
means that timetstepping the equations evolves a terminal condition
...
and $\upert_\tau$ are well contained inside the domain. See \S\,2.1 of
\citet{bbs08a}.
...
\verb+ARPACK+ \citep{lehoucq98} by compling and running \verb+dog-AR+.
