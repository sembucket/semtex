...
uses parametrically mapped quadrilateral elements, and the classic GLL
`nodal' shape function basis.  Algorithmically the code is similar to
Ron Henderson's \textsf{Prism} \citep{hk95,kh98,hen99b}, but with some
differences in design, and lacking mortar element capability. A
notable extension is that \textsf{Semtex} can solve problems in
cylindrical as well as Cartesian coordinate systems \citep{blsh04}.
...
supports the standard libraries. Makefiles assume GNUmake.  SuperMongo
and TECPLOT would be nice to have but are not essential to get up and
running.  All major executables have a \texttt{-h} command line option
which provides a usage prompt.
...
link from \texttt{make} to \texttt{gmake} somewhere.
\item
the BLAS and LAPACK libraries\,---\,these should be either under
\texttt{/usr/lib} or in \texttt{/usr/local/lib}: search for
...
\item
a standard C++ compiler, or a FORTRAN77 compiler.  Note that F90, F95
