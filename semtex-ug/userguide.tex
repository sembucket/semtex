...
\citep[see][]{gms06}. As of \Semtex~V8, the `alternating skew
symmetric' form \citep{zan91b} is the default for construction of
nonlinear terms in the \NavSto\ equations (faster and just as robust
as full skew symmetric, which is still an option), and no dealiasing
of product terms is carried out.  

A significant novelty of \Semtex~V8 is the option of robust outflow
boundary conditions \citep{dkc14}, which alleviate much of the
...
\begin{tabular}{rll}
 $ \bm{f} =$ & $\bm{f}_{const}$                           
  ...
  &  sponge region\\
           $-$ & $m_2(\bm{x})\,\frac{\bm{u}}{|\bm{u}|}\,|\bm{u}(\bm{x}, t)|^2$  
  &  `drag' force  \\
%            + & $\bm{d} \, c$                         &  bouyancy \\
           $+$ & $\bm{\epsilon} G$                        
  &  white noise    \\
           $-$ & 2 $\bm{\Omega} \times \bm{u} - 
                    \frac{d\bm{\Omega}}{dt} \times \bm{x} -
