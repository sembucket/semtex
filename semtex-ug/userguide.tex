...
premium.  In summary, you should always consider trying
\verb+ITERATIVE = 1+ as an option for simulations where the Reynolds
number is more than a few hundred.
...
Nodal elements.
\item
Timestepping algorithm and the code loop. What is stored where, and when.
\item
Boundary conditions, use of inheritance. Distinction between the way
BCs are dealt with. Periodicity not a boundary condition.
\item
...
\item
Implications of Fourier transform in one direction.
\item
...
which excercise most code feauttures.
