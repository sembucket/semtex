...
NEC, Fujitsu, Compaq, SGI, Apple and Linux multiprocessor machines.

There are various code extensions that are not part of the base
distribution. These include dynamic and non-dynamic LES, simple
power-law type non-Newtonian rheologies, scalar transport, buoyancy
via the Boussinesq approximation, accelerating frame of refernce
coupling for aeroelasticity, solution of steady-state flows via
Newton-Raphson iteration, linear stability analysis.
...
%=============================================================================
\section{Particle tracking in steady flows}

This is a very mindless hack. If you want to particle-track in steady
flows, then there is no need to update the Navier--Stokes solution. If
you \verb+make SPAR=1+ you will produce a code version
(\verb+dns_spar+) that will do particle tracking in whatever restart
file is provided. NB: make sure you delete \verb+integrate.o+ first.
