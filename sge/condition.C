...
// ---------------------------------------------------------------------------
{
  ...
  if (_K_ <= EPSSP) {
    sprintf (buf, "transfer coefficient _K_ must be positive (%s)", tok);
...
// used to add _K_*_C_*src to RHS forcing.
// ---------------------------------------------------------------------------
{ 
  const integer  np    = Geometry::nP();
  const integer  nm    = np - 1;
  ...
  Veclib::svvtt (np, _K_*_C_, weight, 1, src, 1, work, 1);
...
void Mixed::augmentSC (const integer  side  ,
		       ...
		       real*          tgt   )  const
// ---------------------------------------------------------------------------
// Add <_K_, w> terms to (banded LAPACK) matrix tgt.
// ---------------------------------------------------------------------------
{
  const integer    np    = Geometry::nP();
  const integer    nm    = np - 1;
...
// <_K_*src, w> to tgt.  Both src and tgt are globally-numbered vectors.
// ---------------------------------------------------------------------------
{
  const integer    np    = Geometry::nP();
  const integer    nm    = np - 1;
...
// BCs.  Add in diagonal terms <_K_, w> to globally-numbered tgt.
// ---------------------------------------------------------------------------
{
  const integer    np    = Geometry::nP();
  const integer    nm    = np - 1;
