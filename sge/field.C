...
void Field::setPatch (MixPatch* patch) const
// ---------------------------------------------------------------------------
// Traverse patch and load data value areas from elemental storage.  This
// sets the mixed BC area for the Field on the other side of the patch
// to access, so data are copied in reverse order.
// ---------------------------------------------------------------------------
{
  const integer           np (Geometry::nP());
  const integer           nz (Geometry::nZProc());
  const vector<Boundary*> BC = bsys -> BCs();
  ListIterator<Patch*>    p (patch -> patches);
  Patch*                  P;
  Boundary*               B;
  vector<real>            work (np);
  real*                   tmp = work();
  ...
  for (p.reset(); p.more(); p.next()) {
    P = p.current();
    for (found = 0, i = 0; !found && i < nbound; i++) {
      B = BC[i];
      if        (found = B == P -> b1) {
	for (k = 0; k < nz; k++) {
	  B -> get (plane[k], tmp);
//	  Veclib::copy (np, tmp, 1, P -> val2 + (k + 1) * np - 1, -1);
	  for (j = 0; j < np; j++) P -> val2[k * np + np - j - 1] = work[j];
	}
      } else if (found = B == P -> b2) {
	for (k = 0; k < nz; k++) {
	  B -> get (plane[k], tmp);
//	  Veclib::copy (np, tmp, 1, P -> val1 + (k + 1) * np - 1, -1);
	  for (j = 0; j < np; j++) P -> val1[k * np + np - j - 1] = work[j];
	}
      }
    }
  }
...
void Field::getPatch (MixPatch* patch)
// ---------------------------------------------------------------------------
// Traverse patch and load BC values for this Field from storage in patch.
// ---------------------------------------------------------------------------
{
  const integer           np (Geometry::nP());
  const integer           nz (Geometry::nZProc());
  ...
  for (p.reset(); p.more(); p.next()) {
    ...
    for (found = 0, i = 0; !found && i < nbound; i++) {
      ...
      if      (found = B == P -> b1)
	for (k = 0; k < nz; k++)
	  Veclib::copy (np, P -> val1 + k*np, 1, line[k] + B -> ID() * np, 1);
      else if (found = B == P -> b2)
	for (k = 0; k < nz; k++)
	  Veclib::copy (np, P -> val2 + k*np, 1, line[k] + B -> ID() * np, 1);
    }
  }
}
