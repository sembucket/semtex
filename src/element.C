...
// ---------------------------------------------------------------------------
{
  ...
  if (Geometry::system() == Geometry::Cylindrical)
    for (register int i = 0; i < _npnp; i++)
...
// ---------------------------------------------------------------------------
{
  ...
  const real   EPS  = (sizeof (real) == sizeof (double)) ? EPSDP : EPSSP;
  const real   hCon = (Geometry::system() == Geometry::Cylindrical && r2 > EPS)
                      ? (betak2 / r2 + lambda2) : betak2 + lambda2;
  register int m, n;
...
// ---------------------------------------------------------------------------
{
  ...
  Veclib::vmul (_npnp, &tmp[0], 1, _G4, 1, &tmp[0], 1);
...
// ---------------------------------------------------------------------------
{
  ...
  register real *dA = _G4, *u = &work[0], *gw = u + _npnp;
  const real    **DV, **DT;
...
void Element::sideGetR (const int side,
			real*         tgt ) const
// ---------------------------------------------------------------------------
