...
void Element::project (const int   nsrc,
		       const real* src ,
		       const int   ntgt,
		       real*       tgt ,
		       real*       work) const
...
// ---------------------------------------------------------------------------
{
  const int     loopcnt = _next; // -- Workaround for NEC vectorisation.
  register int  i, e;
  register real w;
...
void Element::sideGet (const int   side,
		       const real* src ,
		       real*       tgt ) const
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
{
  const int      loopcnt = _npnp; // -- Workaround for NEC vectorisation.
  register int   i;
...
void Element::sideGetY (const int side,
			real*     tgt ) const
// ---------------------------------------------------------------------------
...
void Element::sideMulY (const int   side,
			const real* src ,
			real*       tgt ) const
// ---------------------------------------------------------------------------
...
void Element::sideDivY (const int   side,
			const real* src ,
			real*       tgt ) const
// ---------------------------------------------------------------------------
...
void Element::sideDivY2 (const int   side,
			 const real* src ,
			 real*       tgt ) const
// ---------------------------------------------------------------------------
// Deliver in tgt the side traverse of src (elemental storage) divided
...
// ---------------------------------------------------------------------------
{
  ...
  if (MaxItn== 0) MaxItn = static_cast<int>(Femlib::value ("NR_MAX"));
...
#if defined (DEBUG)
      if (static_cast<int>(Femlib::value("VERBOSE")) > 3) cerr << "N" << endl;
#endif
      return 0;
    }
...
#if defined (DEBUG)
      if (static_cast<int>(Femlib::value("VERBOSE")) > 3) cerr << "D" << endl;
#endif
      return 0;
    }
...
// ---------------------------------------------------------------------------
{
  const int     loopcnt = _npnp; // -- Workaround for NEC vectorisation.
  register int  ij;
  register real tmp, r2, hCon, *R, *S, *r;
  
  R = wrk; S = wrk + _npnp; r = _ymesh;
  ...
  if (_cyl) {
	...
	hCon     = (r2 > EPSDP) ? (betak2 / r2 + lambda2) : 0.0;
	tmp      = R [ij];
	R  [ij]  = _Q1[ij] * R  [ij] + _Q3[ij] * S  [ij];
	S  [ij]  = _Q2[ij] * S  [ij] + _Q3[ij] * tmp;
	tgt[ij]  = _Q4[ij] * src[ij] * hCon;
      }
    } else {
      for (ij = 0; ij < loopcnt; ij++) {
	r2       = r[ij] * r[ij];
