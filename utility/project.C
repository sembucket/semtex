...
// ---------------------------------------------------------------------------
{
  ...
  if (rhs.np == np && rhs.nz == nz)
    ...
    if      (uniform == +1)
      Femlib::projection (&IN, &IT, rhs.np, GRJ, 0.0, 0.0, np, TRZ, 0.0, 0.0);
    else if (uniform == -1) 
      Femlib::projection (&IN, &IT, rhs.np, TRZ, 0.0, 0.0, np, GRJ, 0.0, 0.0);
    else
      Femlib::projection (&IN, &IT, rhs.np, GRJ, 0.0, 0.0, np, GRJ, 0.0, 0.0);
