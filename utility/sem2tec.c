...
static integer nr, ns, nz, nel, nfields, preplot_it = 1, np = 1, dump = 1;
static char    type[MAXFIELDS];
static double  *data[MAXFIELDS], *x, *y, *z;

static void    write_tec   (FILE*);
static void    parse_args  (integer, char**);
static void    read_mesh   (FILE*);
static integer read_data   (FILE*);
static integer interpolate (void);
...
static void parse_args (integer argc,
			char**  argv)
/* ------------------------------------------------------------------------- *
...
static integer read_data (FILE *fp)
/* ------------------------------------------------------------------------- *
 ...
 * ------------------------------------------------------------------------- */
{
  register integer k, m, nplane_new;
  const integer    nrns = nr * ns, nplane_old = nr * ns * nel;
 ...
 * ------------------------------------------------------------------------- */
{
  ...
  const integer    nrns = nr * ns, nplane = nr * ns * nel;
